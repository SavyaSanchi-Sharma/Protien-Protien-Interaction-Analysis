...
**Original run date:** 2026-04-29
**Latest run date:** 2026-05-01

> **Update 2026-05-01 (architecture rev):** multi-layer PLM extraction + per-layer LayerNorm scalar mix + bidirectional cross-attention fusion + tightened HP search. Both `esm_model` and `protbert_model` improved by +0.02–0.04 MCC on each test set. The `o_*` ablations (single-layer, no struct fusion) are unchanged from the original run and are preserved for historical reference. See [REGRESSION_ANALYSIS.md](REGRESSION_ANALYSIS.md), [MULTILAYER_EMBEDDINGS.md](MULTILAYER_EMBEDDINGS.md), and [SOTA_COMPARISON.md](SOTA_COMPARISON.md) for the architecture changes and re-ranked SOTA position.
...
### Latest run (2026-05-01, after architecture rev)

| Test set | Best of our models | Paper MCC | Our MCC | Δ MCC | Paper AUPRC | Our AUPRC | Δ AUPRC |
|---|---|---:|---:|---:|---:|---:|---:|
| **Test_60** | `esm_model` | 42.7 | **50.59** | **+7.9** | 53.1 | **63.01** | **+9.9** |
| **Test_315** | `protbert_model` | 45.3 | **50.09** | **+4.8** | 54.4 | **59.75** | **+5.4** |
...
### Original run (2026-04-29) — historical reference

| Test set | Best | Our MCC | Δ vs paper | Our AUPRC | Δ vs paper |
|---|---|---:|---:|---:|---:|
| Test_60 | `protbert_model` | 47.72 | +5.0 | 59.92 | +6.8 |
| Test_315 | `protbert_model` | 47.31 | +2.0 | 58.33 | +3.9 |
...
| **Ours: `esm_model` (NEW 2026-05-01)** | – | 86.71 | 57.72 | 59.28 | **58.49** | **50.59** | **85.94** | **63.01** |
| **Ours: `protbert_model` (NEW 2026-05-01)** | – | 86.22 | 55.88 | 60.48 | 58.09 | 49.91 | 84.95 | 60.14 |
| Ours: `esm_model` (OLD 2026-04-29) | – | 87.29 | 62.71 | 48.14 | 55.71 | 46.90 | 85.64 | 59.87 |
| Ours: `protbert_model` (OLD 2026-04-29) | – | 85.66 | 54.32 | 57.83 | 56.51 | 47.72 | 85.05 | 59.92 |
| Ours: `o_esm` (ablation, single-layer) | – | 78.75 | 36.66 | 47.47 | 42.20 | 29.49 | 75.18 | 37.57 |
| Ours: `o_protbert` (ablation, single-layer) | – | 78.78 | 31.52 | 29.30 | 36.42 | 21.58 | 68.76 | 29.07 |
...
|---|---:|---:|---:|---:|
| **`esm_model` (NEW 2026-05-01)** | **+6.1** | **+7.9** | **+1.8** | **+9.9** |
| **`protbert_model` (NEW 2026-05-01)** | **+5.7** | **+7.2** | **+0.9** | **+7.0** |
| `protbert_model` (OLD 2026-04-29) | +4.1 | +5.0 | +1.0 | +6.8 |
| `esm_model` (OLD 2026-04-29) | +3.3 | +4.2 | +1.5 | +6.8 |
| `o_esm` (single-layer) | -10.2 | -13.2 | -8.9 | -15.5 |
| `o_protbert` (single-layer) | -16.0 | -21.1 | -15.4 | -24.0 |
...
| **Ours: `protbert_model` (NEW 2026-05-01)** | – | **50.09** | **59.75** |
| **Ours: `esm_model` (NEW 2026-05-01)** | – | 46.95 | 58.26 |
| Ours: `protbert_model` (OLD 2026-04-29) | – | 47.31 | 58.33 |
| Ours: `esm_model` (OLD 2026-04-29) | – | 44.39 | 55.48 |
| Ours: `o_esm` (single-layer) | – | 25.34 | 29.98 |
| Ours: `o_protbert` (single-layer) | – | 17.58 | 24.83 |
...
| Model | MCC Δ | AUPRC Δ |
|---|---:|---:|
| **`protbert_model` (NEW)** | **+4.8** | **+5.4** |
| **`esm_model` (NEW)** | **+1.7** | **+3.9** |
| `protbert_model` (OLD) | +2.0 | +3.9 |
| `esm_model` (OLD) | -0.9 (≈ tied) | +1.1 |
| `o_esm` (single-layer) | -19.96 | -24.42 |
| `o_protbert` (single-layer) | -27.72 | -29.57 |
...
Both `esm_model` and `protbert_model` **decisively exceed ESGTC-PPIS** on every reported metric on both held-out test sets:

- `esm_model`: **+7.9 MCC, +9.9 AUPRC on Test_60** (now best on Test_60)
- `protbert_model`: **+4.8 MCC, +5.4 AUPRC on Test_315** (now best on Test_315)

vs the original 2026-04-29 run, the architecture rev added:
- +0.029 MCC and +0.031 AUPRC on Test_60
- +0.028 MCC and +0.014 AUPRC on Test_315

For where this places us against contemporary 2024–2026 SOTA (we're now ranked 4th–5th of 9), see [SOTA_COMPARISON.md](SOTA_COMPARISON.md).

### Headline contribution of our approach over the paper's

1. Pre-trained PLM embeddings replacing PSSM+HMM
2. **Multi-layer PLM extraction with per-layer LayerNorm scalar mix** (new in 2026-05-01 rev — see [MULTILAYER_EMBEDDINGS.md](MULTILAYER_EMBEDDINGS.md))
3. **Bidirectional cross-attention fusion** replacing scalar gated fusion (new in 2026-05-01 rev)
4. Soft-MCC penalty in the loss
5. MCC-optimal threshold selection
6. MCC-only checkpoint trigger

The 2×2 ablation in §5 reflects the original (single-layer + gated fusion) regime and is preserved as a historical reference; the conclusion that structural fusion is the dominant contributor (+17 to +30 MCC) still stands. The earlier finding that "ProtBert (1024D) beats ESM-2 (2560D) when both have struct fusion" has flipped on Test_60 in the new rev (esm_model now leads at MCC 0.506 vs protbert's 0.499) — likely because the per-layer LayerNorm fix unlocked ESM-2's intermediate-layer signal that was previously dominated by the much-larger-magnitude middle layers.
