...
from test import get_hp, load_models, run_eval, ESM_DIR, TEST_CSVS, CKPT, NUM_WORKERS, TEST_BATCH_SIZE
...
def plot_loss_curves(log, out_path):
    ...
    ax.set_title("Training / Validation loss (esm_model)")
...
def plot_loss_components(log, out_path):
    ...
    fig.suptitle("Loss components (esm_model) — focal drives quality, soft-MCC drives correlation")
    ax1.grid(alpha=0.3)
...
def plot_val_metrics(log, out_path):
    ...
    ax.set_title("Validation metrics over epochs (esm_model)")
...
def plot_pr_curves(probs_dict, labels_dict, out_path):
    ...
    ax.set_title("Precision–Recall curves on test sets (esm_model)")
...
def plot_roc_curves(probs_dict, labels_dict, out_path):
    ...
    ax.set_title("ROC curves on test sets (esm_model)")
    ax.set_xlim(0, 1); ax.set_ylim(0, 1)
    ax.grid(alpha=0.3); ax.legend()
    fig.tight_layout(); fig.savefig(out_path, dpi=120); plt.close(fig)
...
def collect_test_predictions():
    ...
    for name, csv_path in TEST_CSVS.items():
        ...
        ds = PPISDataset(csv_path, ESM_DIR)
...
def write_summary(log, probs_dict, labels_dict, saved_thresh, out_path):
    ...
    lines = []
    lines.append("=" * 72)
    lines.append("esm_model — analysis summary")
    lines.append("=" * 72)
