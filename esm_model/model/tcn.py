...
class BiTCN(nn.Module):
    ...
    def forward(self, x, lengths=None):
        ...
        valid = (torch.arange(L_max, device=x.device).unsqueeze(0)
                 < lengths.unsqueeze(1)).unsqueeze(-1).to(x.dtype)

        xf = self.f(x * valid, mask=valid)
        xb_rev = self.b(flip_per_sequence(x, lengths) * valid, mask=valid)
        xb = flip_per_sequence(xb_rev, lengths)
