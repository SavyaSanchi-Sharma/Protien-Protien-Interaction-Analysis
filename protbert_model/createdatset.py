...
# Must match the layer set in extract_protbert_multilayer.py.
NUM_PLM_LAYERS = 6
PLM_DIM = 1024
...
class PPISDataset(Dataset):
    def __init__(self, csv_path, protbert_dir, cutoff=14.0, norm_path=None):
        ...
        for (pdb, chain), g in self.df.groupby(["PDB", "Chain"]):
            plm_path = self._find_emb_path(pdb, chain, self.protbert_dir)
            if plm_path is None:
                missing += 1
    ...
    def __getitem__(self, idx):
        ...
        if (plm.ndim != 3 or plm.shape[1] != NUM_PLM_LAYERS
            ...
            raise RuntimeError(
                f"Invalid ProtBERT shape at {c['plm_path']}: {tuple(plm.shape)}, "
