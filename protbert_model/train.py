...
def ensure_normalization(train_csv_path, output_path=None):
    ...
    if os.path.exists(output_path):
        log_info(f"Normalization file exists: {output_path}")
        return
    log_info(f"[NORM] Computing normalization from: {train_csv_path}...")
...
def evaluate(loader, alpha_pos):
    proj.eval(); fusion.eval(); gcn.eval(); tcn.eval(); clf.eval()
...
def main():
    global proj, fusion, gcn, tcn, clf

    set_seed(int(os.environ.get("PPI_SEED", "42")))
    hp = load_hp()
    log_info("=" * 80)
    log_info("protbert_model: ProtBERT (1024D) + 17D structural | GCN + BiTCN")
    ...
    train_ds = PPISDataset(TRAIN_CSV, PROTBERT_DIR, PDB_DIR)
    val_ds   = PPISDataset(VAL_CSV,   PROTBERT_DIR, PDB_DIR)
    ...
    proj   = ProtBertProjection(1024, hidden_dim=512, out_dim=256, dropout=hp["proj_dropout"]).to(DEVICE)
    fusion = GatedFusion(d_esm=256, d_struct=17, d_out=256, dropout=hp["fusion_dropout"]).to(DEVICE)
    ...
    total_params = sum(p.numel() for m in [proj, fusion, gcn, tcn, clf] for p in m.parameters())
    ...
    for epoch in range(EPOCHS):
        proj.train(); fusion.train(); gcn.train(); tcn.train(); clf.train()
        epoch_loss = 0.0
        ...
        if improved:
            patience_count = 0
            torch.save({
                "proj": proj.state_dict(), "fusion": fusion.state_dict(),
                "gcn": gcn.state_dict(), "tcn": tcn.state_dict(),
                "clf": clf.state_dict(), "epoch": epoch + 1,
