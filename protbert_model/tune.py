...
from train import (TRAIN_FULL_CSV, TRAIN_CSV, VAL_CSV, PROTBERT_DIR,
...
def build(trial):
    hp = {
        "lr":             trial.suggest_float("lr", 6e-6, 2.5e-5, log=True),
        "weight_decay":   trial.suggest_float("weight_decay", 1e-4, 5e-3, log=True),
        "focal_gamma":    trial.suggest_float("focal_gamma", 1.5, 2.5),
        "proj_dropout":   trial.suggest_float("proj_dropout", 0.10, 0.30),
        "fusion_dropout": trial.suggest_float("fusion_dropout", 0.05, 0.25),
        "fusion_heads":   trial.suggest_categorical("fusion_heads", [2, 4]),
        "gcn_layers":     trial.suggest_int("gcn_layers", 4, 8),
        "gcn_alpha":      trial.suggest_float("gcn_alpha", 0.25, 0.60),
        "gcn_dropout":    trial.suggest_float("gcn_dropout", 0.05, 0.20),
        "tcn_dropout":    trial.suggest_float("tcn_dropout", 0.05, 0.20),
        "clf_dropout":    trial.suggest_float("clf_dropout", 0.15, 0.35),
        "lambda_mcc":     trial.suggest_float("lambda_mcc", 0.8, 2.0),
        "batch_size":     2,
    }
    modules = [
        MultiLayerProjection(NUM_PLM_LAYERS, PLM_DIM, 512, 256,
...
def main():
    ...
    train_ds = PPISDataset(TRAIN_CSV, PROTBERT_DIR)
    val_ds   = PPISDataset(VAL_CSV,   PROTBERT_DIR)
