...
def load_models(ckpt, hp):
    ...
    for m in (proj, gcn, tcn, clf):
        m.eval()
    return proj, gcn, tcn, clf


@torch.no_grad()
def run_eval(loader, proj, gcn, tcn, clf):
...
def main():
    ...
    hp = get_hp()
    proj, gcn, tcn, clf = load_models(ckpt, hp)
    saved_thresh = float(ckpt.get("optimal_threshold", 0.5))
    print(f"[TEST] Saved threshold: {saved_thresh:.4f}")

    results = {}
    for name, csv_path in TEST_CSVS.items():
        if not os.path.exists(csv_path):
            print(f"[SKIP] {name}: {csv_path} not found"); continue
        ds = PPISDataset(csv_path, ESM_DIR, PDB_DIR)
        loader = DataLoader(ds, batch_size=1, shuffle=False)
        probs, labels = run_eval(loader, proj, gcn, tcn, clf)
